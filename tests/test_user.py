...
class MigrationsTest(TestCase):
    ...
    def test_monkey_patch_db_column(self):
        ...
        table_fields = connection.introspection.get_table_description(
            cursor, table_name
        )
        field = next(
            field for field in table_fields if field.name == "last_login"
        )  # pragma: no cover
...
@skipIf(
    SessionAuthenticationMiddleware is None,
    "SessionAuthenticationMiddleware not available in this version",
)
